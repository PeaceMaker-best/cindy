 ...
 *  - **排除表优先于放行表**（`device-link:mediaFetch` 落在放行根下但必须被拦）。
 */
...
describe('放行：基础设施来源', () => {
  ...
  it('两种子 scope 分隔符都认（仓库里 / 与 : 都在用）', () => {
    expect(isAllowedScope('device-link/foo')).toBe(true);
    expect(isAllowedScope('device-link:ipc')).toBe(true);
  });
});
...
describe('排除表优先于放行表', () => {
  it.each(__testing.DENIED_SUB_SCOPES)('%s 被拦（隶属放行根但会带本地路径/媒体内容）', (scope) => {
    expect(isAllowedScope(scope)).toBe(false);
  });
  ...
  it('排除表里的每一项都确实落在某个放行根下（否则这条排除是死代码，说明名单写错了）', () => {
    for (const denied of __testing.DENIED_SUB_SCOPES) {
      const root = denied.split(/[/:]/)[0];
      expect(__testing.ALLOWED_ROOT_SCOPES).toContain(root);
    }
  });
});
