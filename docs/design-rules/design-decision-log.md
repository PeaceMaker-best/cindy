...
- **08-03** **排版立法:字重梯改四档、桌面字号白名单建档(拍板人 = 用户,issue #1505)**——
  ...
  状态下已实际漂移——`font-semibold`(600)×73、`font-bold`(700)×11、`font-extrabold`(800)×1,
  任意值字号 `text-[Npx]` 约 550 处(含 22 处小数、8 处 <10px);而手机端 2026-07 已收敛到
  ...
  600 收编存量 73 处 semibold(合法化、不改代码),700 只许经豁免登记表(markdown `<strong>` /
  ...
  {18, 20, 24, 28};只做小数吸附、<10px 升下限、任意值归 `text-<n>` token、
  `tailwind.config.ts` 删零使用档(9/19/21/22/23/25/26/27)。任意值归 token 的附带修复:
